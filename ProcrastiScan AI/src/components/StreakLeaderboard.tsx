...
export default function StreakLeaderboard({ userName, theme }: StreakLeaderboardProps) {
  ...
  // Helper to fetch consolidated leaderboard from server
      ...
      if (response.ok) {
        ...
        if (data.leaderboard) {
          const mapped = data.leaderboard.map((item: any) => ({
            ...item,
            isCurrentUser: item.name.toLowerCase() === userName.toLowerCase()
          }));
          // Sort descending by streak count
          mapped.sort((a: any, b: any) => b.streak - a.streak);
          setLeaderboard(mapped);
        }
      }
    } catch (e) {
      console.error("Gagal sinkronisasi data leaderboard dengan server:", e);
    }
  };

  // Helper to post current user's streak status to the server
  const syncUserStreakToServer = async (currentStreakValue: number) => {
    if (!userName) return;
    try {
      ...
      await fetch("/api/leaderboard/update", {
        ...
        body: JSON.stringify({ name: userName, streak: currentStreakValue, role })
      });
    } catch (e) {
      console.error("Gagal mengunggah update info streak ke server:", e);
    }
  };
  ...
  useEffect(() => {
    ...
    // Sync streak with server and fetch initially
    const currentStreakValue = currentStreakObj.count;
    syncUserStreakToServer(currentStreakValue).then(() => {
      fetchLeaderboard();
    });

    // Set up a 3-second polling interval for instant real-time synchronization of other users' entries or claps
  ...
  const handleCheckInNow = () => {
    ...
    syncUserStreakToServer(nextStreakCount).then(() => {
      fetchLeaderboard();
    });
  ...
  const handleClapPeer = async (peerName: string, event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    // Optimistic visual update for immediate responsive feel
    setLeaderboard(prev => {
      return prev.map(p => {
        if (p.name === peerName) {
    ...
    // Persist clap count to server
    try {
      const response = await fetch("/api/leaderboard/clap", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      console.error("Gagal mengirim clap ke server:", e);
    }
  };
