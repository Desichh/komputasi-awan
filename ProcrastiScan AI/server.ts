...
// Helper function to query Gemini models with robust automatic fallback (e.g. if gemini-3.5-flash hits active quotas)
  ...
  for (const model of models) {
    try {
      ...
      const isQuotaOrServerError = 
        ...
        errorMsgStr.includes("Quota") || 
        errorMsgStr.includes("limit") || 
        errorMsgStr.includes("demand") || 
        errorMsgStr.includes("429") || 
        errorMsgStr.includes("503");
      
      if (isQuotaOrServerError) {
        console.warn(`[Gemini SDK] Model ${model} failed (Quota/Load). Error details: ${error.message || error}`);
        console.warn(`[Gemini SDK] Attempting next fallback model in line...`);
        continue;
      } else {
        throw error;
      }
