...
export default function AnalyticsDashboard({ 
  ...
  // 15-minute plan timer state variables
    ...
    if (plannerRunning) {
      interval = setInterval(() => {
        setPlanSeconds((prev) => {
          ...
          // Calculate phase: Phase 1 (first 5m: 15:00-10:00), Phase 2 (second 5m: 10:00-05:00), Phase 3 (third 5m: 05:00-00:00)
  ...
  const handleStartPlanTimer = () => {
    if (plannerPausedAt) {
      ...
      if (elapsedSeconds > 120) {
        ...
        setActivePlanPhase(1);
        alert("Jeda melebihi 2 menit! Demi melatih disiplin fokus Anda, rencana aksi ini diulang kembali dari awal.");
      }
      setPlannerPausedAt(null);
    } else if (planSeconds === 0) {
      // Completed: restart from 15 mins
      setPlanSeconds(15 * 60);
      setActivePlanPhase(1);
    }
    ...
    <div id="analytics-dashboard-panel" className="space-y-8">
      ...
      {analysis && !loading && (
        <div id="ai-results-block" className="space-y-8">
          ...
          {/* Interactive 15-Minute Action Planner widget */}
          <div id="action-planner-panel" className="bg-slate-900 border border-teal-500/20 rounded-2xl p-6 shadow-xl relative overflow-hidden font-sans">
            <div className="absolute top-0 right-0 p-3 text-right">
              <span className="font-mono text-3xl font-bold text-teal-400 tracking-widest block bg-slate-950 px-3 py-1 rounded border border-slate-800">
                {formatPlanTime(planSeconds)}
              </span>
              {!plannerRunning && plannerPausedAt && (
                <span className="text-[9px] font-mono text-rose-400 block mt-1 animate-pulse font-semibold">
                  ⏸️ JEDA (Maks 2m)
                </span>
              )}
            </div>
            ...
            <div className="max-w-xl mb-4">
              <h3 className="text-md font-sans font-bold text-slate-100 flex items-center gap-2">
                ...
                Rencana Aksi 15 Menit Penyembunyi Malas
              </h3>
              <p className="text-xs text-slate-300 mt-1 leading-relaxed">
                AI telah merumuskan panduan 15 menit ke depan yang sangat personal khusus untukmu. Jangan dipikirkan, cukup selesaikan!
              </p>
            </div>

            {/* Structured analysis output displayed broken down stepped card lists */}
            <div className="space-y-3 mb-6">
              {(() => {
                const text = analysis.rencana15Menit || "";
                
                // Parse helper to extract 3 distinct sequential time block cards
                const parsePlans = (rawText: string) => {
                  ...
                  if (rawLines.length > 1) {
                    ...
                    rawLines.forEach((line) => {
                      ...
                      if (isTimeBlockHeader) {
                        ...
                        // Clean up markdown/bold characters from title
                        activeTitle = line.replace(/[*#_\-•]|(###)/g, "").trim();
                        activeContent = [];
                      } else {
                  ...
                  // If parsing resulted in too few items or empty, do robust fallback sentence/phase splitter
                  if (list.length < 2) {
                    ...
                    if (partsByMarker.length > 2) {
                      for (let i = 1; i < partsByMarker.length; i += 2) {
                        ...
                        const phaseBody = (partsByMarker[i + 1] || "").replace(/^[\s\:\;\-\.,]+/, "").trim();
                        if (phaseBody) {
                          list.push({ title: phaseTitle, content: phaseBody });
                        }
                      }
                    } else {
                      // Split by sentence ends as elegant fallback
                      const sentences = cleanText.split(/[.!?]+/).map(s => s.trim()).filter(s => s.length > 2);
                      if (sentences.length >= 3) {
                        list.push({ title: "Menit 1-5 (Fase Awal)", content: sentences.slice(0, Math.ceil(sentences.length / 3)).join(". ") + "." });
                        list.push({ title: "Menit 6-10 (Fase Tengah)", content: sentences.slice(Math.ceil(sentences.length / 3), Math.ceil(2 * sentences.length / 3)).join(". ") + "." });
                        list.push({ title: "Menit 11-15 (Fase Akhir)", content: sentences.slice(Math.ceil(2 * sentences.length / 3)).join(". ") + "." });
                      } else {
                        list.push({ title: "Menit 1-15 (Rencana Cepat)", content: cleanText });
                      }
                    }
                  }
                ...
                const steps = parsePlans(text);
                
                return steps.map((step, idx) => {
                  let badgeColor = "bg-teal-950 text-teal-400 border-teal-500/30";
                  if (step.title.includes("6-10") || step.title.includes("Tengah") || idx === 1) {
                    badgeColor = "bg-amber-950 text-amber-400 border-amber-500/30";
                  } else if (step.title.includes("11-15") || step.title.includes("Akhir") || idx === 2) {
                  ...
                  return (
                    <div
                      key={idx}
                      className="p-4 rounded-xl border border-slate-800/80 bg-slate-950/60 shadow-xs flex flex-col sm:flex-row sm:items-start gap-4 transition-all hover:bg-slate-950/90 hover:border-teal-500/30 group"
                    >
                      <div className={`sm:w-36 flex-shrink-0 flex items-center justify-center py-1.5 px-3 rounded-lg border text-center font-mono font-bold text-[10px] md:text-xs uppercase tracking-tight select-none ${badgeColor}`}>
                        ⏱️ {step.title}
                      </div>
                      <div className="flex-1">
                        <p className="text-xs text-slate-200 leading-relaxed font-sans group-hover:text-white transition-colors">
                          {step.content}
                        </p>
                      </div>
                    </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-4 border-t border-slate-800/60">
              <span className="text-[11px] font-mono text-slate-400 italic">
                {plannerRunning 
                  ? "⏱️ Hitung mundur rencana aksi 15 menit sedang berjalan..." 
                  : (planSeconds < 15 * 60 
                      ? "⏸️ Rencana aksi dijeda. Klik Lanjutkan!" 
                      : "Siap menaklukkan tugas raksasa? Klik jalankan timer!"
                    )
                }
              </span>
              ...
              <div className="flex gap-2">
                {plannerRunning ? (
                  <button 
                    ...
                    className="text-amber-400 hover:text-amber-300 font-mono text-xs px-3 py-2 rounded bg-amber-950/20 hover:bg-amber-950/30 border border-amber-900/40 transition-colors cursor-pointer"
                  >
                    Jeda Rencana
                  </button>
                ) : (
                  <button
                    ...
                    className="bg-teal-500 hover:bg-teal-400 text-slate-950 font-sans font-bold text-xs px-4 py-2.5 rounded-lg flex items-center gap-2.5 shadow-lg shadow-teal-500/20 transition-all cursor-pointer"
                  >
                    <Play className="w-4 h-4 fill-slate-950" />
                    {planSeconds < 15 * 60 ? "Lanjutkan Rencana!" : "Mulai Rencana 15 Menit!"}
                  </button>
                )}
              </div>
            </div>
            ...
            {plannerRunning && (
              <div className="mt-4 grid grid-cols-3 gap-2 text-center text-[10px] font-mono">
                <div className={`p-2 rounded border transition-all ${activePlanPhase === 1 ? "bg-teal-950/40 border-teal-500 text-teal-300" : "bg-slate-950 border-slate-850 text-slate-500"}`}>
                  Fase 1 (Menit 1-5)
                  <span className="block text-[8px] opacity-70">Persiapan Mikro</span>
                </div>
                <div className={`p-2 rounded border transition-all ${activePlanPhase === 2 ? "bg-teal-950/40 border-teal-500 text-teal-300" : "bg-slate-950 border-slate-850 text-slate-500"}`}>
                  Fase 2 (Menit 6-10)
                  <span className="block text-[8px] opacity-70">Matikan Gangguan</span>
                </div>
                <div className={`p-2 rounded border transition-all ${activePlanPhase === 3 ? "bg-teal-950/40 border-teal-500 text-teal-300" : "bg-slate-950 border-slate-850 text-slate-500"}`}>
                  Fase 3 (Menit 11-15)
                  <span className="block text-[8px] opacity-70">Aksi Terkecil</span>
                </div>
              </div>
            )}
